...
let wordsMap = sonnet.match(/[\w']+/g);

for (let i = 0; i < wordsMap.length; i++) {
  let wordMap = wordsMap[i];
  let currentValue = uniqueMap.get(wordMap);

  if (uniqueMap.get(wordMap)) {
    uniqueMap.set(wordMap, currentValue+1);
  } else {
    uniqueMap.set(wordMap, 1);
  }
}
